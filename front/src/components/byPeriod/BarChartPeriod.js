...
class BarChartPeriod extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    const dataByPeriod = this.props.dataByPeriod;
    let barChart = [];
    // At the beginning there is no dataByPeriod or during a fetching
    if (dataByPeriod && dataByPeriod.length > 0) {
      let sortData = this.sortData(dataByPeriod);
      barChart.push(
...
BarChartPeriod.PropTypes = {
  sortedBy: PropTypes.string.isRequired,
  dataByPeriod: PropTypes.object.isRequired,
